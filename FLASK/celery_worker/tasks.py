...
def sync_google_drive(
    storage_strings: dict, id_diag: str, email: str, drive_inst, mongo_inst, coll_diags
):
    print(f"STORAGE: {storage_strings}, ID: {id_diag}")
    path_video_in = storage_strings["video_in"]
    path_video_out = storage_strings["video_out"]
    id_in = drive_inst.upload_to_drive(path_video_in, [email])
    id_out = drive_inst.upload_to_drive(path_video_out, [email])

    # Update the document in the database with the new Google Drive file IDs
    mongo_inst.db.get_collection(coll_diags).update_one(
...
def processamento_analise(
    ...
    timestamp = time.time()
    diag = mongo_inst.db.get_collection(coll_diags).find_one({"_id": ObjectId(id_diag)})
    ...
    os.rename(
        os.path.join(flask_app.config["TEMP_FOLDER"], nome_input),
        os.path.join(flask_app.config["TEMP_FOLDER"], filename_arq_input),
    )

    # video deve ser armazenado usando ID do usuario e timestamp, pra garantir multiplicidade
    path_arq_input = os.path.join(flask_app.config["TEMP_FOLDER"], filename_arq_input)
    ...
    path_aux_conv = os.path.join(
        flask_app.config["TEMP_FOLDER"], f"CONVERT_{nome_local}"
    )
    ...
    path_out_antes_conv = os.path.join(
        flask_app.config["TEMP_FOLDER"], f"OUT_PRE_{nome_local}"
    )
    ...
    path_out = os.path.join(flask_app.config["TEMP_FOLDER"], f"OUT_{nome_local}")
    ...
    #     flask_app.config["TEMP_FOLDER"], f"RELATORIO_{nome_video}.pdf")
    ...
    if str(id_medico) != "None":
        ...
        medico = Helper.find_one_with_id(
            mongo_inst.db.get_collection(coll_meds), id_medico
        )
...
def envia_diag_task(
    ...
    if user_id:
        medicos = mongo_inst.db.get_collection(coll_meds)
    ...
    diags = mongo_inst.db.get_collection(coll_diags)
    result = diags.insert_one(dados)
    id_diag_mongo = str(result.inserted_id)

    nome_local = f"{str(round(timestamp, 4))}_{filename}"
    path_temp_videoLabel = os.path.join(flask_app.config["TEMP_FOLDER"], nome_local)
