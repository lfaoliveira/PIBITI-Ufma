...
from flask_backend.yolo3.utils import compose
...
def DarknetConv2D_BN_Leaky(*args, **kwargs):  # DarkConv Aqui
...
def resblock_body(x, num_filters, num_blocks):  # DarkBlock Aqui
    ...
    for i in range(num_blocks):
        ...
        x = Add()(
            [x, y]
        )  # bloco residual aqui: soma os resultados de cada camada de convolução, resultando em uma camada com mesmo shape
    return x
...
def yolo_head(feats, anchors, num_classes, input_shape, calc_loss=False):
    ...
    box_xy = (K.sigmoid(feats[..., :2]) + grid) / tf.cast(
        grid_shape[..., ::-1], tf.float32
    )
    box_wh = (
        K.exp(feats[..., 2:4])
        * anchors_tensor
...
def yolo_eval(
    ...
    # Concatenar todas as caixas e pontuações
    boxes = tf.concat(boxes, axis=0)
    box_scores = tf.concat(box_scores, axis=0)

    # Aplicar threshold de pontuação
    ...
    for c in range(num_classes):
        ...
        class_ids = tf.ones_like(class_box_scores, dtype=tf.int32) * c
    ...
    if boxes_:
        ...
        # Retornar tensores vazios com shape apropriado
        boxes_ = tf.zeros((0, 4), dtype=tf.float32)
        scores_ = tf.zeros((0,), dtype=tf.float32)
        classes_ = tf.zeros((0,), dtype=tf.int32)
